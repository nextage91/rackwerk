...
/** Tippen per pointerdown/pointerup statt 'click' -- Browser erzeugen ein
 *  synthetisches 'click' NUR für den PRIMÄREN aktiven Touch-Punkt; ein
 *  zweiter, GLEICHZEITIG gehaltener Finger (z. B. während man mit der
 *  anderen Hand das X/Y-Pad zieht) bekommt pointerdown/pointerup ganz
 *  normal, aber nie ein 'click' -- per echtem Multitouch-Test (zwei
 *  simultane Touch-Punkte über CDP) bestätigt: der Stop-Button feuerte
 *  pointerdown/pointerup zuverlässig als zweiter Finger, aber niemals
 *  click. Jedes Jam-Bedienelement, das auch WÄHREND einer laufenden
 *  Pad-Geste antippbar sein soll, bindet deshalb hierüber (Nutzer-Anfrage:
 *  "während ich das X/Y-Pad bediene, kann ich keine andere Touch-Aktion
 *  ausführen"). Bricht ab, wenn der Finger sich weiter als TAP_THRESHOLD
 *  bewegt hat, bevor losgelassen wird -- sonst würde ein Wisch/Scroll-
 *  Versuch, der zufällig auf demselben Element endet, als Tap zählen.
 *  Elemente mit einer EIGENEN, komplexeren Tap-vs-Halten/Ziehen-Logik
 *  (Clips: Umsortieren + Löschen-Menü, s. makeReorderable; Scene-Chips:
 *  Löschen-Menü, s. wireSceneChip) lösen ihren Tap direkt aus ihrer
 *  eigenen pointerup-Auswertung aus statt bindTap() zu nutzen -- dieselbe
 *  Idee, nur bereits mit der zusätzlichen Gesten-Unterscheidung verwoben. */
function bindTap(el, handler) {
  let startX = 0, startY = 0, pointerId = null, moved = false;
  el.addEventListener('pointerdown', (e) => {
    if (e.button != null && e.button > 0) return; // nur Primärtaste/Touch, keine rechte Maustaste etc.
  ...
  el.addEventListener('pointermove', (e) => {
    ...
    if (Math.hypot(e.clientX - startX, e.clientY - startY) > TAP_THRESHOLD) moved = true;
  });
  ...
  el.addEventListener('pointercancel', (e) => { if (e.pointerId === pointerId) pointerId = null; });
}
...
/** Aktualisiert den Anker (Pad-Mitte, s. anchorFor()) EINES gemappten
 *  Parameters auf `value` -- egal auf welcher Achse er gerade sitzt (oder
 *  gar nicht gemappt ist, dann No-Op). Nutzer-Anfrage: eine bewusste
 *  Weiterentwicklung des Sound-Designs NACH dem Zuordnen -- ob am echten
 *  Rack-Regler (s. trackHandDrag()) oder über den Jam-Makro-Knopf (s.
 *  buildMacros()) -- soll automatisch die neue Pad-Mitte werden, ohne dass
 *  man extra "Set center to current value" antippen muss. Bewusst NICHT
 *  bei jedem generischen 'input'-Event (das würde auch LFO-/Automations-
 *  Ticks auf demselben Regler mitreissen, s. Dateikopf/Chat) -- nur diese
 *  zwei EXPLIZITEN, tatsächlich von Hand ausgelösten Wege rufen das hier
 *  auf. */
function reanchorIfMapped(machine, key, value) {
  const st = machine.xyMap;
  if (!st) return;
  for (const axis of ['x', 'y']) {
    const entry = st[axis]?.find((e) => e.key === key);
    if (entry) entry.anchor = value;
  }
}

/** Hängt (einmalig pro echtem Regler-Element, s. grabTrackedKnobs) einen
 *  'knob-release'-Listener an -- das Event, das <x-knob> selbst NUR am
 *  Ende einer echten Zieh-Geste feuert (dasselbe, das automation.js für
 *  "Hand schlägt Automation" nutzt), nie bei einem synthetisch gesetzten
 *  Wert (Pad, Makro-Knopf, LFO, Automation-Lane -- die schreiben alle nur
 *  .value + ein 'input'-Event, ohne die Geste zu simulieren). Bleibt der
 *  Listener über die gesamte Lebenszeit des Regler-Elements bestehen
 *  (Maschinen-Panels werden nie neu gebaut, s. machine.js#render()) --
 *  reagiert also auch dann noch, wenn ein Regler mal ent- und wieder
 *  gemappt wird, ohne einen zweiten Listener zu brauchen. */
const grabTrackedKnobs = new WeakSet();
function trackHandDrag(machine, key, knob) {
  if (!knob || grabTrackedKnobs.has(knob)) return;
  grabTrackedKnobs.add(knob);
  knob.addEventListener('knob-release', () => {
    reanchorIfMapped(machine, key, parseFloat(knob.value));
  });
}
    ...
    if (otherKeys.has(key)) {
      ...
      btn.addEventListener('click', () => {
        ...
        if (mixKey && !newEntries.some((e) => e.key === mixKey) && !otherKeys.has(mixKey)) {
          ...
          trackHandDrag(machine, mixKey, mixMeta?.knob);
        }
        trackHandDrag(machine, key, meta?.knob);
        st[axis] = newEntries;
        onChange();
        renderXYMenu(machine, axis, anchorEl, onChange);
      });
...
function renderClips(machine, clipsEl) {
  ...
  // Tippen zum Starten/Stoppen läuft für Clips NICHT über 'click' -- s.
  // makeReorderable() unten, die den Tap direkt aus ihrer eigenen
  // pointerup-Auswertung heraus auslöst (Browser erzeugen 'click' NUR für
  // den PRIMÄREN Touch, s. Dateikopf-Kommentar bei bindTap()).
...
/** Verhindert, dass makeReorderable() auf demselben `clipsEl` mehrfach
 *  volle Listener-Sätze aufbaut -- renderClips() (und damit dieser Aufruf)
 *  läuft bei JEDEM Hinzufügen/Löschen eines Clips erneut, `clipsEl` selbst
 *  bleibt dabei aber das GLEICHE, dauerhafte Element (nur sein innerHTML
 *  wird ersetzt). Ohne diese Sperre stapelten sich mit jedem weiteren Clip
 *  weitere, komplett unabhängige pointerdown/move/up-Listener übereinander
 *  -- bei zwei oder mehr Clips verarbeitete dann JEDER Satz dieselbe Geste
 *  parallel, und ein SPÄTERER Satz sah beim eigenen pointerup bereits die
 *  vom ERSTEN Satz (der zuerst fertig lief) entfernte 'is-dragging'-Klasse
 *  -- fälschlich als "war kein Ziehen" gewertet, löste zusätzlich zum
 *  Umsortieren noch ein ungewolltes toggleClip() aus (per Reproduktion
 *  bestätigt: Ziehen zum Umsortieren startete/stoppte dabei nebenbei den
 *  gezogenen Clip). Bestand schon vor dem Umbau auf direktes toggleClip()
 *  unten, wurde vorher nur durch das separate 'click'+suppressClick-Timing
 *  zufällig seltener sichtbar. */
const reorderableWired = new WeakSet();
 ...
 *  Löst ausserdem den Start/Stop-Tap selbst aus (s. release() unten) --
 *  weder über ein separates 'click' noch über ein suppressClick-Flag:
 *  ein einfacher Tap (weder gezogen noch fürs Löschen-Menü gehalten) ruft
 *  toggleClip() direkt an genau der Stelle auf, die ohnehin schon weiss,
 *  ob eine der beiden Gesten stattfand -- funktioniert dadurch auch als
 *  ZWEITER, gleichzeitig gehaltener Finger (Browser erzeugen 'click' nur
 *  für den PRIMÄREN Touch, s. bindTap()-Kommentar; Nutzer-Anfrage: "während
 *  ich das X/Y-Pad bediene, kann ich keine andere Touch-Aktion ausführen"). */
function makeReorderable(clipsEl, machine) {
  if (reorderableWired.has(clipsEl)) return;
  reorderableWired.add(clipsEl);
  ...
  const release = (e) => {
    ...
    // Nur bei einem ECHTEN Loslassen (nicht bei pointercancel) und weder
    // gezogen noch fürs Löschen-Menü gehalten -- ein einfacher Tap.
    if (e.type === 'pointerup' && !wasDragging && !heldForDelete) {
      toggleClip(machine, Number(tappedEl.dataset.clipId));
    }
...
function buildXYPad(machine) {
  ...
  const st = xyStateFor(machine);
  // Hand-Drag-Tracking (s. trackHandDrag()) für ALLE aktuell gemappten
  // Einträge sicherstellen -- deckt insbesondere aus einem gespeicherten
  // Projekt geladene Zuordnungen ab, die nie über renderXYList()s Add-
  // Button liefen (dort wird zusätzlich sofort getrackt, s. dort).
  for (const axis of ['x', 'y']) {
    for (const entry of st[axis]) {
      trackHandDrag(machine, entry.key, readKnobMeta(machine, entry.key)?.knob);
    }
  }
...
function buildMacros(machine) {
  ...
  for (const key of params) {
    ...
    knob.addEventListener('input', (e) => {
      ...
      // Bewusstes Nachjustieren über den Jam-Makro-Knopf zählt genau wie
      // ein echter Rack-Regler-Drag (s. trackHandDrag()) -- der Anker
      // zieht mit, falls dieser Parameter gerade aufs X/Y-Pad gemappt ist.
      reanchorIfMapped(machine, key, e.detail.value);
    });
...
function buildColumn(machine) {
  ...
  bindTap(soloBtn, () => { machine.setSoloed(!machine.soloed); soloBtn.classList.toggle('is-active', machine.soloed); });
  bindTap(muteBtn, () => { machine.setMuted(!machine.muted); muteBtn.classList.toggle('is-active', machine.muted); });
...
/** Kuratierte Makro-Parameter für den Master-Kanal (Gegenstück zu
 *  MACRO_PARAMS je Maschinentyp oben) -- die vier Sends/Effekt-Regler, die
 *  sich live am meisten zum "Aufdrehen" für einen Break/Drop eignen
 *  (Delay-/Reverb-Sendpegel, Delay-Feedback, Reverb-Decay). Bewusst NUR
 *  die eingebauten Delay/Reverb-Regler, keine Insert-Effekt-Parameter --
 *  genau wie bei den Maschinen-Makros (auch dort tauchen nur die
 ...
 *  availableXYParams()) -- nur Reso/Sweep sind fest verdrahtet.
 *
 *  Nutzer-Anfrage (2. Folge-Wunsch, nach dem ersten Filter-Umbau): Sweep
 *  soll keine kleine Encoder-Zelle mehr sein, sondern eine eigene Touch-
 *  Fläche wie das X/Y-Pad -- nur eine Achse (Y). Der Master-Kanal hat
 *  keinen Fader, der den Platz sonst bräuchte; dieses Pad füllt genau
 *  den Rest-Platz, den .fader-row bei den Maschinen-Spalten bekäme
 *  (s. .sweep-wrap in app.css, `flex: 1 1 auto` wie .fader-row). */
function buildSweepPanel() {
  const sweepMeta = readKnobMeta(masterFX, 'filterSweep');
  const resoMeta = readKnobMeta(masterFX, 'filterReso');
  ...
  if (resoMeta) {
    const resoKnob = document.createElement('x-knob');
    resoKnob.setAttribute('label', resoMeta.label);
    resoKnob.setAttribute('min', resoMeta.min);
    resoKnob.setAttribute('max', resoMeta.max);
    if (resoMeta.curve) resoKnob.setAttribute('curve', resoMeta.curve);
    if (resoMeta.unit) resoKnob.setAttribute('unit', resoMeta.unit);
    resoKnob.setAttribute('value', resoMeta.value);
    resoKnob.addEventListener('input', (e) => {
      nudgeParam(resoMeta.knob, e.detail.value);
      reanchorIfMapped(masterFX, 'filterReso', e.detail.value);
    });
    head.appendChild(resoKnob);
  }
  // Auto-Return-Taster (s. fx.js#filterSweepSpring-Kommentar am
  // Konstruktor) -- geteilter Zustand mit dem Rack-Panel-Regler, hier
  // eigens verdrahtet, weil das Pad ein komplett eigenständiges
  // Bauteil ist (kein <x-knob>, kein knob-release-Event).
...
/** Fester "Master"-Kanal ganz am Ende der Jam-Kanalliste (wie das feste
 *  Master-FX-Panel am Ende des Rack-Views) -- keine Clips/kein Stop/keine
 *  Solo-Mute-Reihe/kein Fader (keins davon hat für den Summenbus eine
 *  Entsprechung), aber X/Y-Pad UND Makro-Knobs GENAU wie bei den
 *  Maschinen-Spalten (Nutzer-Anfrage: Master-Effekte während des Jams
 *  genauso live bespielen können). buildXYPad()/xyStateFor() sind bereits
 *  generisch genug für masterFX (nur auf .el/.xySpring/.xyMap angewiesen,
 *  s. dortige Kommentare + fx.js#MasterFX-Konstruktor) -- keine Sonder-
 *  behandlung nötig, nur ein Master-spezifischer XY-Default (s. u.), weil
 *  der generische Default (sendDelay/sendReverb) auf masterFX gar nicht
 *  existiert (Master sendet nicht an sich selbst). */
function buildMasterColumn() {
  if (!masterFX.xyMap) {
    const delayMeta = readKnobMeta(masterFX, 'delayLevel');
    const revMeta = readKnobMeta(masterFX, 'revLevel');
    masterFX.xyMap = {
      x: [{ key: 'delayLevel', from: 0, to: 1, anchor: parseFloat(delayMeta?.value ?? '0.5') }],
      y: [{ key: 'revLevel', from: 0, to: 1, anchor: parseFloat(revMeta?.value ?? '0.4') }],
    };
  }
  const col = document.createElement('div');
  col.className = 'channel channel--master';
  col.style.setProperty('--ch-color', '#d8c9a3');
  col.style.setProperty('--ch-glow', 'rgba(216,201,163,.5)');
  col.innerHTML = `
    <div class="channel__head">
      <span class="channel__stripe"></span>
      <div class="channel__name">Master</div>
    </div>
  `;
  col.appendChild(buildXYPad(masterFX));
  col.appendChild(buildSweepPanel());
  return col;
}
...
function wireSceneChip(chip, scene) {
  ...
  const cancelHold = () => { clearTimeout(holdTimer); holdTimer = null; };
  // Löst den Tap direkt hier aus (nicht über 'click', s. bindTap()-
  // Kommentar) -- funktioniert dadurch auch als zweiter, gleichzeitig
  // gehaltener Finger (z. B. während man mit der anderen Hand das
  // X/Y-Pad einer Spur zieht).
  chip.addEventListener('pointerup', () => {
    const wasHeld = held;
    cancelHold();
    if (!wasHeld) launchScene(scene);
  });
...
export function renderJamView(listEl, scenesContainerEl) {
  ...
  // Ganz am Ende, wie das feste Master-FX-Panel am Ende des Rack-Views.
  listEl.appendChild(buildMasterColumn());
