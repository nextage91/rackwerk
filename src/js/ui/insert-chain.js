 ...
 *  kann"). `eq8QToDb`/`eq8DbToQ` bilden Q auf dieselbe dB-Y-Achse ab wie
 *  Gain (Q ist selbst schon ein Verhältnis, 20·log10(Q) ist die dazu
 *  passende log-Skala) -- so kann der Punkt auf EXAKT derselben Achse wie
 *  ein normales Band gezeichnet/gezogen werden, ohne eine zweite, eigene
 *  Achsen-Skalierung zu brauchen. */
const eq8IsCutType = (b) => b.type === 'highpass' || b.type === 'lowpass';
const eq8QToDb = (q) => 20 * Math.log10(q);
const eq8DbToQ = (db) => Math.max(EQ8_Q_MIN, Math.min(EQ8_Q_MAX, 10 ** (db / 20)));
