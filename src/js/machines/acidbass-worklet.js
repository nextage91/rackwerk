...
class AcidBassProcessor extends AudioWorkletProcessor {
  ...
  #onParamsChanged() {
    ...
    // lineare Skalierung (s. erste Korrektur, Chat).
    //
    // ZWEITE Korrektur (s. Chat): die erste Ober-Grenze (1.6 normal / 2.2
    // Hi-Res) klang zwar bei Ohr-Tests nach "mehr Resonanz", schwang aber
    // NIE wirklich hörbar auf: per Zeitmessung (Standalone-Sweep, Zeit bis
    // die Rückkopplung eine hörbare Amplitude erreicht) brauchte diese
    // Ober-Grenze bei typischen Cutoff-Einstellungen (400-2000Hz) SEKUNDEN
    // bis NIEMALS statt der für eine 303-Bassline (kurze, schnelle Noten)
    // nötigen wenigen Millisekunden -- das Selbstschwingen ist zwar
    // mathematisch vorhanden, kam aber in der Praxis nie rechtzeitig an,
    // bevor die Note längst wieder verklungen war. Diese höhere Ober-Grenze
    // (4.5 normal / 7.0 Hi-Res) wurde genau daraufhin kalibriert: bei voll
    // aufgedrehter Resonanz erreicht die Selbstschwingung über den gesamten
    // Cutoff-Reglerbereich hinweg eine hörbare Amplitude innerhalb von
    // <20ms (oft <5ms) -- fühlt sich jetzt wie das erwartete "sofortige"
    // 303-Kreischen an, nicht wie ein Regler ohne Wirkung. Der
    // Sicherheits-Clip unten (Tanh im Feedback-Pfad + weicher Zustands-
    // Clamp) fängt jeden Rest-Fall ab, in dem die reine (komplett lineare)
