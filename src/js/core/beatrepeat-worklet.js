 ...
 *  - Treffer (< chance) UND es gibt bereits eine eingefrorene Scheibe:
 *    dieselbe Leseposition wie beim letzten Mal wird erneut abgespielt
 *    (nicht neu eingefangen) -- repeatCount steigt, die Lautstärke sinkt
 *    um (1-decay)^repeatCount.
  ...
  process(inputs, outputs, parameters) {
    ...
    for (let i = 0; i < n; i++) {
      ...
      if (this.cyclePos === 0) {
        ...
        if (this.hasCapturedBefore && Math.random() < chance) {
          this.repeating = true;
          this.repeatCount += 1;
      ...
      for (let ch = 0; ch < output.length; ch++) {
        if (this.repeating) {
          ...
          output[ch][i] = this.ring[ch % this.ring.length][idx] * this.gain;
        } else {
          const inCh = input && input.length ? input[ch % input.length] : null;
