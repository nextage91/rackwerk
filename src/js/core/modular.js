...
const MODULE_DEFS = {
  oscillator: {
    ...
    build(ctx, p) {
      ...
      osc.type = p.wave;
      // Bewusst NICHT mit einer hörbaren Tonhöhe (z. B. C4) starten: der
      // Oscillator läuft seit Modul-Erstellung durchgehend, bevor je eine
      // Note gespielt wurde -- normalerweise unhörbar, weil der VCA bis
      // zum ersten Envelope-Trigger komplett zu ist (s. VCA#build). Auf
      // manchen Geräten reproduzierbar (Nutzer-Bugreport): GENAU beim
      // allerersten je ausgelösten Trigger ist kurz die bis dahin
      // geltende (Default-)Tonhöhe zu hören, bevor sie auf die Zielnote
      // springt -- ein setValueAtTime()-Priming direkt beim Bauen (frühere
      // Fassung) hat das NICHT behoben, der genaue Mechanismus auf dem
      // betroffenen Gerät bleibt unklar. Statt weiter zu raten, WARUM kurz
      // etwas durchsickert: einfach dafür sorgen, dass das, was in diesem
      // Fenster durchsickern könnte, keine hörbare Tonhöhe hat -- 1 Hz
      // liegt weit unter dem Hörbereich, klingt also wie Stille statt wie
      // eine falsche Note, ganz gleich was den genauen Kaltstart-Effekt
      // auslöst.
   ...
   *
   * cancelAndHoldAtTime() statt cancelScheduledValues()+setValueAtTime
   * (0.0001, t): weil dieselbe Instanz für JEDE Note wiederverwendet wird
   * (s. oben), kann eine neue Note feuern, WÄHREND die vorige noch mitten
   * im (evtl. langen) Release ist -- cancelScheduledValues() allein
   * bricht die laufende Rampe an ihrem AKTUELLEN Wert ab (z. B. 0.6 bei
   * halb durchlaufenem Release), das direkt folgende setValueAtTime
   * (0.0001, t) sprang von dort aber HART auf nahe Null, bevor die neue
   * Attack-Rampe wieder hochlief -- ein hörbares Klicken (Chat: "klicken
   * bei langen Release-Zeiten bevor die nächste Note spielt", per
   * Reproduktion bestätigt: ein Sprung von ~0.6 auf ~0 und zurück auf 1
   * innerhalb von unter 10ms). cancelAndHoldAtTime hält den Wert exakt
